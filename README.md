...
Travel website allows the user who has not yet traveled to Iceland, to find more about his destination of travel and what they would like to visit. 
Using two search bars on the website they can pick location and nearby places they would like to search for, the website will then display this destination on google map and show locations of interest to the user. 
...
- Strategy plane:
    ...
    - As someone looking to travel to Iceland, I want to use the website to find out more about travel destinations of my interest.
    - As a user of the travel website, I want to be able to search for more locations and go through articles. 
    - As a user of the travel website, I want to have an insight into what different cities have to offer in terms of attractions, hotels, food, and restaurants...
    - As a user of the travel website, I would like to contact the website admin in case I have any questions.
 
- Scope plane:
    - Users should be able to search for their location using the search bar.
    - There should be articles about different tourist attractions in Iceland. 
    - Articles should be displayed on the location page after the user has provided the input and clicked the "search" button.
...
- Structure plane:
    - User Interface:
        ...
        - Users can search for destinations, and places in Iceland using two input fields.
        - Google Map API service will provide the user with information about places that he/she has searched. 
...
- Main feature:
    ...
    - The main features of this project are the two search bars that allow the user to input data that is then used by Google API services, Google Map, Geolocation, and GooglePlaces.
...
- Three other features:
    ...
    - Ajax: 
        - On the second successful request where geolocation has initiated a function is run that uses ajax method to get JSON data from [assets/ajax](https://github.com/Manojlovic1998/Second_Milestone_Project/blob/master/assets/ajax/articles.json).
        This function gets the data and uses a template provided on the bottom of the location.html page to run a loop that queries the template, makes a copy of it, inputs data to the copy of the template, and appends the template to an empty container.
        This way articles can be added inside the JSON file and there won't be any need to create new elements as the loop does this for us using the premade template.

- EmailJS:
    - The website has a contact feature available on the contactus.html page, where users can use the form to send us an email. This feature was set using [EmailJS](https://www.emailjs.com/) service.
    The user gets to fill 3 input fields, "Full Name", "Email", and "Message". Once all the input fields have been filled, the user can click the send button which takes the data and uses the EmailJS service to send us an email using the template that we have set on Emailjs to format the data accordingly. 
 
 - Intersection Observer:
    - Intersection Observer API is used to load in text and images, creating a fade-in animation that is made by utilizing CSS style and JS logic through a callback function that uses the API observer to set and remove classes. 

### Existing Features

- Google API - allows users to search for cities within Iceland and find nearby places of interest by inputting data inside two search boxes on the "locations.html" page.
- Email JS 2 - allows the user to send an email to the admin of the page by inputting data inside a form on the "contactus.html" page.
- Intersection Observer - allows the user to scroll down the page and reveal more content with a smooth transition that adds to the professional look and feel of the website.

### Features Left to Implement

- Initial plan when wireframes were created was to have only one page where once an anchor is clicked the page will load new content in place of the old one. 
A Good Example of this would be, once the navigation link is clicked then only new content of the page is loaded instead of having the whole page load again. For this, I would use the JSON file, template elements, and JS logic.
What this would allow me to do is that I would be able to change the content easily inside a JSON file without ever having to do it through HTML. So in case, I want to change images, article texts, article headings I could do it all just by updating the JSON file.
This would also allow me to create a new feature where after reading an article about some location, the user can click a button that will automatically initiate a function that will load the map and display the location of the place that the user has just read about.

- Due to technical issues, an excessive database that would have to be built, and the short deadline for the first website deployment, this feature has not made it into the deployed version. 
However, the website will be updated soon, where this feature will be included.
...
It also allowed displaying a website in a horizontal phone view, this way I found a problem with height units, "vh", that I initially used to set the background image.
I have fixed the issue by replacing the unit, using rem instead of vh.
...
Once the repository was created, I have used browser IDE addon for GitHub called [GitPod](https://chrome.google.com/webstore/detail/gitpod-online-ide/dodmmooeoklaejobgleioelladacbeki?hl=en), to open the repository.
...
8. In the top part of the "GitHub Pages" section you will find text saying "Your site is published at ..." and it will include the link to the deployed website.
9. Try to click on the link, it should open the deployed version of your website.
10. In case the browser failed to load the website, you need to wait 2-5 minutes as sometimes it knows to take longer for the website to be deployed.
11. After you have waited 2-5min, you can try to open the website again using the link found in the "GitHub Pages" section.
12. The website is now deployed and online.


- Difference between the development version and the deployed version is that:
 1. In the deployed version, image jpg files have been downsized so that they get loaded faster.
 2. Google Maps API key was restricted after the website was deployed to prevent any misuse.
 3. New articles have been added to the deployed version compared to the development.
 ...
 5. In the deployed version location page contains a section with articles about cities that users might want to search for.
